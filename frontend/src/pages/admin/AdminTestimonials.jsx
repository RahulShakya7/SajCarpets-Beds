import { Pencil, Trash, Plus, UploadSimple, Star } from "@phosphor-icons/react";
...
export default function AdminTestimonials() {
    ...
    const [formData, setFormData] = useState({ name: "", location: "", content: "", rating: 5, image: null });
    ...
    return (
        <div>
            ...
            <Modal isOpen={isEditing} onClose={() => setIsEditing(false)} title={editId ? "Edit Testimonial" : "New Testimonial"}>
                {error && (
                        ...
                        <div className="flex items-center gap-2">
                            {[1, 2, 3, 4, 5].map((star) => (
                                <button
                                    ...
                                    className={`p-1 transition-colors ${formData.rating >= star ? 'text-yellow-400' : 'text-gray-300 dark:text-gray-600'}`}
                                >
                                    <Star size={24} weight={formData.rating >= star ? "fill" : "regular"} />
                                </button>
                            ))}
                        </div>
                    </div>
                    ...
                    <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                        {loading ? (
                            ...
                            <tr key={item.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 text-gray-800 dark:text-gray-200">
                                ...
                                <td className="px-6 py-4 flex items-center text-yellow-400 gap-1">{item.rating} <Star weight="fill" /></td>
